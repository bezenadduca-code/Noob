...
-- FIXED: Better WindUI loader with fallback
local function loadWindUI()
    ...
    -- If all fails, try alternative URL
...
pcall(function()
    win = ui:CreateWindow({
        Title          = "V1PRWARE",
        Icon           = "sparkles",
        Author         = "V1pr & Mitsuki",  -- CHANGED: Updated author
        Folder         = "v1prware",
...
-- FIXED: Use WindUI's built-in open button (works on PC and mobile)
pcall(function()
    win:EditOpenButton({
        Title          = "V1PRWARE",
        Icon           = "sparkles",
        ...
        OnlyMobile = false,  -- Changed: Works on PC too
        Enabled    = true,
        Draggable  = true,
    })
end)

-- Add a close button using WindUI's built-in method if available
pcall(function()
    if win and win.Close then
        -- Some WindUI versions have a Close method
        win.Close = function()
...
secNosferatu:Toggle({ Title="Auto QTE", Type="Checkbox", Flag="qteOn", Default=qte.on,
    Callback=function(on) pcall(function() qte.on=on; if on then qteStart() else qteStop() end end) end })

secNosferatu:Slider({ Title="Click Speed (s)", Flag="qteSpeed", Step=0.005,
    Value={Min=0.005, Max=0.2, Default=qte.speed},
    Callback=function(v) pcall(function() qte.speed=v end) end })
...
secNosferatu:Toggle({ Title="Silent Aim (Bats)", Type="Checkbox", Flag="silentAimOn", Default=silentAim.on,
    Callback=function(on) pcall(function() silentAim.on = on end) end })
...
secNosferatu:Slider({ Title="Prediction (s)", Flag="silentAimPrediction", Step=0.05,
    ...
    Callback=function(v) pcall(function() silentAim.prediction = v end) end })

secNosferatu:Slider({ Title="Aim Offset", Flag="silentAimOffset", Step=0.5,
    Value={Min=-5, Max=5, Default=silentAim.aimOffset},
    Callback=function(v) pcall(function() silentAim.aimOffset = v end) end })
...
-- FIXED: Close UI button that works on PC
secUIFunctions:Button({ 
    Title = "Close UI", 
    Callback = function()
        pcall(function()
            -- Try multiple methods to close
            if win then
                -- Method 1: Try to destroy the window
                pcall(function() win:Destroy() end)
                -- Method 2: Try to hide it
                pcall(function() win.Visible = false end)
                -- Method 3: Try WindUI's close method if available
                pcall(function() win:Close() end)
            end
            -- Also try to close the entire UI
            pcall(function() ui:Destroy() end)
        end)
    end 
})

-- Add a keyboard shortcut for closing (ESC)
pcall(function()
    svc.Input.InputBegan:Connect(function(input, gameProcessed)
