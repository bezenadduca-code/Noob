...
-- FIXED: WindUI open button works on PC too
pcall(function()
    win:EditOpenButton({
        Title          = "V1PRWARE",
        Icon           = "sparkles",
...
-- ===== NOSFERATU SECTION - FIXED AUTO QTE =====
local secNosferatu = tabKiller:Section({ Title = "Nosferatu", Opened = true })

local qte = {
    on = false,
    speed = 0.03,  -- Default: 0.03 = very fast, 0.05 = mid, 0.08 = human-like
}

local qteThread = nil
local qteBtn = nil  -- Cache the button for speed
...
local function qteGetButton()
    ...
    -- Look for ActiveButton specifically (this is the actual button name)
...
local function qtePressButton(btn)
    ...
    pcall(function()
        -- Method 1: Direct event fire (works reliably)
        btn.MouseButton1Click:Fire()
        btn.MouseButton1Down:Fire()
        btn.MouseButton1Up:Fire()
        btn.Activated:Fire()
        
        -- Method 2: getconnections fallback (for extra reliability)
    ...
    qteThread = task.spawn(function()
        while qte.on do
            pcall(function()
                ...
                if btn then
                    qtePressButton(btn)
                    task.wait(qte.speed)  -- Speed slider controls this
                else
                    task.wait(0.05)  -- Fast scan if no button
                end
            end)
        end
        qteThread = nil
...
secNosferatu:Slider({ 
    ...
    Callback = function(v) 
        pcall(function() 
            qte.speed = v 
            -- Update speed display based on value
            local speedText = ""
            if v <= 0.02 then
                speedText = "⚡ Very Fast"
            elseif v <= 0.05 then
            ...
            print("[QTE] Speed set to " .. string.format("%.3f", v) .. "s - " .. speedText)
        end) 
    end 
})
...
function applyConfigFromFlags()
    pcall(function()
        ...
        -- QTE Config (FIXED)
        if win.Flags.qteOn ~= nil then 
            qte.on = win.Flags.qteOn
            if qte.on then 
                qteStart() 
            else 
                qteStop() 
        ...
        if type(win.Flags.qteSpeed) == "number" then 
            qte.speed = win.Flags.qteSpeed
        end
